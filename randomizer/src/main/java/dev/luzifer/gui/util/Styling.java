package dev.luzifer.gui.util;

public final class Styling {
    
    public static final String CLEAR = "";
    public static final String BASE = "-fx-base: #9aa5aa;";
    public static final String BASE_DARKER = "-fx-base: #77868c;";
    public static final String BACKGROUND = "-fx-background-color: #9aa5aa;";
    public static final String BACKGROUND_DARKER = "-fx-background-color: #77868c;";
    public static final String BACKGROUND_BANZAI_BLUE = "-fx-background-color: #3168a9;";
    public static final String BACKGROUND_BANZAI_BLUE_LIGHTER = "-fx-background-color: #4783ca;";
    public static final String DROP_SHADOW = "-fx-effect: dropshadow(gaussian, rgba(0, 0, 0, 0.3), 10, 0.5, 0.0, 0.0);";
    public static final String HEADER = "-fx-background-color: #688c9b;";
    public static final String CONTENT = "-fx-background-color: #93a8b1;";
    public static final String SELECTED = "-fx-background-color: #c6cccf;";
    public static final String UNSELECTED = "-fx-background-color: #77868c;";
    public static final String BORDER = "-fx-border-color: #404240;";
    public static final String BORDER_RED = "-fx-border-color: #c60000; -fx-border-width: 5px;";
    public static final String BOTTOM_BORDER_BLUE = "-fx-border-color: #00c6ff; -fx-border-width: 0px 0px 5px 0px;";
    public static final String FONT_RED = "-fx-text-fill: #c60000;";
    
    private Styling() {
    }
    
}
